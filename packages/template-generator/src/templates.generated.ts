...
`],
  ...
  "exports": {
    ...
    "./convex": {
      "default": "./convex/_generated/api.d.ts"
    }
  },
  "scripts": {
    "dev": "convex dev",
...
{{#if (eq payments "polar")}}
    "@polar-sh/sdk": "^0.42.2",
{{/if}}
{{#if (and (eq payments "polar") (eq auth "better-auth"))}}
...
{{/if}}
{{#if (eq payments "polar")}}
    "@convex-dev/polar": "^0.7.3"{{#if (includes examples "ai")}},{{/if}}
{{/if}}
{{#if (includes examples "ai")}}
...
export const env = createEnv({
	server: {
{{#if (and (ne database "none") (ne database "convex"))}}
{{#if (eq dbSetup "planetscale")}}
...
`],
  ["db/convex/src/index.ts.hbs", `import { ConvexHttpClient } from "convex/browser";
import { env } from "@{{projectName}}/env/server";

// HTTP client for use in API routes (Hono, Express, etc.)
// This allows you to call Convex functions from your backend
...
export default defineConfig({
  ...
  ["db/convex/convex/convex.config.ts.hbs", `import { defineApp } from "convex/server";

const app = defineApp();

export default app;
`],
  ["db/convex/convex/README.md", `# Convex Database
...
`],
  ["db/convex/convex/schema.ts.hbs", `import { defineSchema, defineTable } from "convex/server";
import { v } from "convex/values";
...
`],
  ["db/convex/convex/tsconfig.json.hbs", `{
  "compilerOptions": {
    "target": "ESNext",
...
`],
  ["db/convex/convex/_gitignore", `_generated/
`],
...
{{#if (eq payments "polar")}}
// Initialize Polar client for Better Auth integration
...
`],
  ["auth/clerk/convex/backend/convex/http.ts.hbs", `{{#if (eq payments "polar")}}
...
{{/if}}
	 ...
	 * TODO: Replace these placeholder IDs with your actual Polar product IDs from the dashboard
	 * @see https://docs.polar.sh/ for setup instructions
	 */
	products: {
		// TODO: Replace with your actual Polar product IDs before production
...
export const authClient = createAuthClient({
	...
	plugins: [
		...
		convexClient(){{#if (eq payments "polar")}}, polarClient(){{/if}},
	],
});
`],
...
`],
  ["payments/polar/convex/native/bare/components/subscription-status.tsx.hbs", `// Re-export from base - bare uses the same component structure
export { SubscriptionStatus } from "../base/components/subscription-status";
`],
  ["payments/polar/convex/native/uniwind/components/subscription-status.tsx.hbs", `// Re-export from base - uniwind uses the same component structure
export { SubscriptionStatus } from "../base/components/subscription-status";
`],
...
`],
  ["payments/polar/convex/native/base/components/subscription-status.tsx.hbs", `import { useState } from "react";
...
`],
  ["payments/polar/convex/native/unistyles/components/subscription-status.tsx.hbs", `// Re-export from base - unistyles uses the same component structure
export { SubscriptionStatus } from "../base/components/subscription-status";
`],
...
export const authClient = createAuthClient({
	...
	plugins: [convexClient(), crossDomainClient(){{#if (eq payments "polar")}}, polarClient(){{/if}}],
});`],
...
export const authClient = createAuthClient({
	plugins: [convexClient(){{#if (eq payments "polar")}}, polarClient(){{/if}}],
});`],
...
`],
  ["payments/polar/convex/web/react/tanstack-router/src/routes/success.tsx.hbs", `import { createFileRoute, useSearch } from "@tanstack/react-router";
import { useQuery } from "convex/react";
import { api } from "../../../../convex/_generated/api";
...
`],
  ["payments/polar/convex/web/react/tanstack-start/src/routes/success.tsx.hbs", `import { createFileRoute, useSearch } from "@tanstack/react-router";
import { useQuery } from "convex/react";
import { api } from "../../../../convex/_generated/api";
	...
	validateSearch: (search) => ({
		checkout_id:
			typeof search.checkout_id === "string"
				? search.checkout_id
				: Array.isArray(search.checkout_id) && typeof search.checkout_id[0] === "string"
					? search.checkout_id[0]
					: undefined,
	}),
});
...
`],
  ["payments/polar/convex/web/react/react-router/src/routes/success.tsx.hbs", `import { useSearchParams } from "react-router";
import { useQuery } from "convex/react";
import { api } from "../../../../convex/_generated/api";
...
`],
  ["payments/polar/convex/web/react/next/src/app/success/page.tsx.hbs", `"use client";
...
export const TEMPLATE_COUNT = 452;

...
Better-T-Stack generates all Alchemy infrastructure through one `packages/infra/alchemy.run.ts` artifact assembled from a typed deployment plan. The current integration pins `alchemy@2.0.0-beta.72` with Effect beta.107 and emits Cloudflare, Prisma, managed-database, mixed-provider, full-stack `self`, and separate-server topologies. Generated paths are not automatically live-verified support claims; the accepted-version scoreboard defined below owns that distinction.
...
| A11 | retired: older Effect compatibility pin                 | removed release workaround            | beta.72 starts with Effect beta.107; fresh Bun, npm, and pnpm graphs resolve without an override            |
...
The product surface calls the target **Prisma**; `Prisma.Compute` is the Alchemy resource used in generated code. Supported automatic framework builds are Next.js, Nuxt, Astro, and TanStack Start. SolidStart uses its verified `.output/server/index.mjs` artifact. React Router uses the framework's documented custom-server Vite input so `react-router build` emits a self-contained Express server at `build/server/index.js`. SvelteKit uses `adapter-node`, then a second Vite SSR build bundles the standalone server and copies its client/prerendered directories into `.prisma`. TanStack Router's Vite SPA output is paired with a bundled `sirv` production server and SPA fallback in `.prisma`; `vite preview` is intentionally not used because Vite documents it as a local preview tool. Server apps use the Bun auto-build surface for Hono, Express, Fastify, and Elysia on Bun or Node.

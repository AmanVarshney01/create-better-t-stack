...
The product surface calls the target **Prisma**; `Prisma.Compute` is the Alchemy resource used in generated code. Supported automatic framework builds are Next.js, Nuxt, Astro, and TanStack Start. SolidStart uses its verified `.output/server/index.mjs` artifact. React Router uses the framework's documented custom-server Vite input so `react-router build` emits a self-contained Express server at `build/server/index.js`. SvelteKit uses the official `adapter-node` output directly at `build/index.js`; Vite bundles its production dependencies during that framework build because Prisma Compute uploads only the selected artifact directory. TanStack Router is rejected for Prisma deployment because it emits only static SPA assets and the released Alchemy Prisma provider has no static-site resource. Server apps use the Bun auto-build surface for Hono, Express, Fastify, and Elysia on Bun or Node.

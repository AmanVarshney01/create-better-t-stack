...
<Callout type="info">
  If a project combines Vercel with Cloudflare (which owns the bare `deploy` script via Alchemy),
  these scripts are generated with a `:vercel` suffix instead: `link:vercel`, `env:vercel:preview`,
  `env:vercel:production`, `deploy:vercel`, `deploy:vercel:prod`, `deploy:vercel:check`.
</Callout>

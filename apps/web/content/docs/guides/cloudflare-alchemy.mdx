...
<Callout type="info">
  Generated projects pin `alchemy@2.0.0-beta.77` with Effect rc.112 exactly. The framework source
  package is installed in the web workspace that Alchemy loads it from. Root dependency overrides
  keep Alchemy's transitive Effect packages on rc.112 across Bun, npm, and pnpm.
</Callout>
...
Alchemy beta.77 still uses APIs removed in Effect rc.113. The generated root dependency overrides
keep its transitive Effect platform and SQL packages on rc.112 as well. Upgrade these packages
together once Alchemy supports the new Effect APIs, and commit the resulting lockfile.

...
- exact Alchemy and Effect version pins
...
server. TanStack Router uses Alchemy's automatic Vite build, which packages static assets with
Prisma Compute's HTTP entrypoint and enables SPA route fallback.
...
for React Router, TanStack Router, TanStack Start, and Solid. Next.js and stable SvelteKit use
`StaticSite` with OpenNext and SvelteKit's Cloudflare adapter, respectively.
...
Drizzle templates use Drizzle ORM and Drizzle Kit `1.0.0-rc.4`. Each generated migration has its own directory containing `migration.sql` and `snapshot.json`, which Alchemy applies during deployment. Better Auth uses its official Drizzle Relations v2 adapter.
...
<Callout type="info">
  Generated projects pin `alchemy@2.0.0-beta.79` with `effect@4.0.0-rc.115` exactly. The matching
  framework source package is installed in the workspace that loads it.
</Callout>

Frontend-only Next.js, Nuxt, Astro, SvelteKit, TanStack Start, and TanStack Router projects use
Alchemy’s `Prisma.Website` integrations for development, production builds, and runtime packaging.
Full-stack projects, React Router, Solid, and separate API servers use `Prisma.Compute`.
...
Alchemy v2 is a beta. Better-T-Stack pins Alchemy, `@alchemy.run/frontend-frameworks`, Effect, and
the Effect platform packages to exact versions. The package-manager lockfile records the resolved

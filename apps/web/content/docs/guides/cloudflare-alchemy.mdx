...
<Callout type="info">
  Generated projects pin `alchemy@2.0.0-beta.78` with Effect rc.115 exactly. The matching framework
  source package is installed in the web workspace that Alchemy loads it from. This release supports
  Effect rc.115 without the transitive dependency overrides required by beta.77.
</Callout>
...
Effect release candidate, currently `4.0.0-rc.115`, because open prerelease ranges can select
unintended publications. Transitive dependencies can still resolve newer release candidates, so
installation and typechecking alone are insufficient: verify that the Alchemy CLI starts too.
When upgrading an existing beta.77 project, update Alchemy, `@alchemy.run/frontend-frameworks`,
Effect, and both Effect platform packages together. Remove the old rc.112 overrides for
`@effect/platform-node-shared`, `@effect/sql-d1`, `@effect/sql-sqlite-do`, and `@effect/vitest`
from the root `package.json` (or `pnpm-workspace.yaml` for pnpm), then reinstall and commit the
resulting lockfile. Update generated infrastructure calls from `Config.string` and
`Config.redacted` to `Config.String` and `Config.Redacted`, respectively.

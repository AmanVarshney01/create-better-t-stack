...
- `alchemy.run.ts` for web and server applications
- `database.ts` for Alchemy-managed database resources and migrations
- exact Alchemy and Effect versions verified by Better-T-Stack
...
Two Next.js combinations are temporarily unavailable on Cloudflare because of current OpenNext
packaging bugs:

- Sentry, until OpenNext Cloudflare includes the instrumentation tracing fix
- Prisma with a generic `node-postgres` connection, including PlanetScale Postgres and Supabase,
  until OpenNext preserves `pg-cloudflare`'s workerd runtime files
...
<Callout type="warn">
  ...
  `packages/infra/database.ts` before deploying.
</Callout>
...
For Sentry, runtime DSNs go to the deployed app while organization, project, and auth-token values are available only to the web build that uploads source maps. See the [Sentry guide](/docs/guides/sentry).
...
Alchemy v2 is still a beta. Better-T-Stack pins one exact Alchemy version together with exact compatible Effect packages, because open prerelease ranges can select incompatible publications. Upgrade these packages as a verified set rather than changing one dependency independently.

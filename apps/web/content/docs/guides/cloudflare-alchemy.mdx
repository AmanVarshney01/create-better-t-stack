...
- web: Next.js, Nuxt, Astro, TanStack Start, and SolidStart
...
for React Router, TanStack Router, TanStack Start, and SolidStart. Next.js and stable SvelteKit keep
...
```bash
# Full-stack SolidStart on Prisma with PlanetScale MySQL
...
For Nuxt and Astro on Cloudflare, Alchemy owns the framework build and injects its Cloudflare
integration in memory. Those web packages intentionally omit a standalone `build` script; use the
root `deploy` command for the actual Cloudflare build.

<Callout type="warn">
  The currently published `alchemy@2.0.0-beta.70` cannot start with the latest Effect beta because
  it calls an Effect API removed in beta.104. The fix is merged upstream but not released, so this
  integration remains blocked until a containing Alchemy release passes the full generated-project
  and live matrix. Better-T-Stack does not downgrade Effect, patch dependencies, or rely on hoisting
  to hide the incompatibility.
</Callout>
...
Alchemy v2 is still a beta. Better-T-Stack pins one exact Alchemy version together with the exact
latest Effect beta because open prerelease ranges can select unintended publications. A pinned set
is not called compatible until the CLI starts and the complete generated-project and live gates
pass. Upgrade these packages as one verified set rather than changing one dependency independently.

...
- `alchemy`: Alchemy platform deployment
...
- `none`: No deployment setup
- `workers`: Cloudflare Workers deployment (when runtime is workers)
- `alchemy`: Alchemy platform deployment (when runtime is workers)

```bash
create-better-t-stack --server-deploy workers

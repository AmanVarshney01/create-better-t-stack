...
- `vercel`: Vercel web deployment using Vercel CLI and `apps/web/vercel.ts`

```bash
create-better-t-stack --web-deploy vercel
```

**Note:** Cloudflare deployments use Alchemy to define infrastructure programmatically. See the [Deploying to Cloudflare with Alchemy Guide](/docs/guides/cloudflare-alchemy) for details.

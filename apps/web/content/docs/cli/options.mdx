...
```bash
create-js-stack --package-manager bun
```
...
create-js-stack --no-render-title
```
...
create-js-stack my-app --yes --directory-conflict overwrite

# Safely create a new directory name if it exists
create-js-stack my-app --yes --directory-conflict increment
```
...
create-js-stack --disable-analytics

# Enable analytics collection (default)
create-js-stack --no-disable-analytics
```

Analytics help improve Create-JS-Stack by providing insights into usage patterns. When disabled, no data is collected or transmitted.
...
```bash
create-js-stack --database postgres
```
...
```bash
create-js-stack --database postgres --orm drizzle
```
...
```bash
create-js-stack --database postgres --db-setup neon
```
...
```bash
create-js-stack --backend hono --runtime bun
```
...
```bash
# Single web frontend
create-js-stack --frontend tanstack-router

# Web + native frontend
create-js-stack --frontend next native-uniwind

# Backend-only
create-js-stack --frontend none
```
...
```bash
create-js-stack --auth better-auth
create-js-stack --auth clerk
create-js-stack --auth none
```
...
```bash
create-js-stack --payments polar --auth better-auth
```
...
- `mcp`: Install MCP servers, including Create JS Stack itself, with add-mcp
...
```bash
create-js-stack --addons pwa biome husky
```
...
```bash
create-js-stack --examples todo ai
```
...
```bash
create-js-stack --web-deploy cloudflare
```
...
```bash
create-js-stack --server-deploy cloudflare
```
...
- **macOS**: `~/Library/Application Support/create-js-stack/history.json`
- **Linux**: `~/.local/share/create-js-stack/history.json`
- **Windows**: `%LOCALAPPDATA%\create-js-stack\Data\history.json`

...
Prefer just the tools without the plugin? Wire the MCP server into Codex:

```bash
npx create-better-t-stack@latest add --addons mcp   # choose "codex"
```
...
```bash
npx create-better-t-stack@latest add --addons mcp   # choose "opencode"
```
...
- **MCP-capable agents** (Cursor, VS Code, Gemini CLI, Zed, and more) — run `npx create-better-t-stack@latest add --addons mcp` and pick your agent.

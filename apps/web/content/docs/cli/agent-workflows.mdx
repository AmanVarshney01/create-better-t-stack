...
Create-JS-Stack now supports a fully JSON-first workflow for agents and automation:
...
```bash
create-js-stack create-json --input '{
  "projectName": "my-app",
...
```bash
create-js-stack add-json --input '{
  "projectDir": "./my-app",
...
Run Create-JS-Stack itself as a local MCP server over stdio:
...
This is the best path when an MCP-capable client should scaffold or modify Create-JS-Stack projects without shelling out to a prompt-driven CLI flow.
...
If you scaffold a project with the `mcp` addon, Create-JS-Stack itself is now one of the recommended MCP servers. The addon installs it through `add-mcp` with a package runner command so the generated config does not rely on a globally installed CLI:

```bash
npx -y add-mcp@latest "npx -y create-js-stack@latest mcp"
```

For Bun projects, the generated config uses the equivalent `bunx create-js-stack@latest mcp` server command inside `add-mcp`. That means MCP-capable agents inside the generated project can talk back to Create-JS-Stack without any extra global install, alongside other recommended docs, framework, and database MCP servers.
...
Structured addon options are still persisted in `cjs.jsonc`, but the printed reproducible command now stays on normal CLI flags for consistency. That means deeply nested addon or provider-specific setup options are not fully encoded in the one-line replay command.
...
```bash
create-js-stack --yes --dry-run
create-js-stack create-json --input '{"projectName":"my-app","yes":true,"dryRun":true}'
create-js-stack add-json --input '{"projectDir":"./my-app","addons":["mcp"],"dryRun":true}'
```
...
Create-JS-Stack persists structured configuration in `cjs.jsonc`, including:

- `reproducibleCommand`
- `addonOptions`
- `dbSetupOptions`

See [`cjs.jsonc`](/docs/cjs-config) for the file format.

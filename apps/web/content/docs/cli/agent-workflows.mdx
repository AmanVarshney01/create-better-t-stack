...
If you scaffold a project with the `mcp` addon, Better-T-Stack itself is now one of the recommended MCP servers. The addon installs it through `add-mcp` as a local stdio command:

```bash
create-better-t-stack mcp
```

That means MCP-capable agents inside the generated project can talk back to Better-T-Stack through the same local CLI command, alongside other recommended docs, framework, and database MCP servers.

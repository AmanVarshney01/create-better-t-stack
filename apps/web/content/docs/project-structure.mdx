...
Electrobun adds an `apps/desktop` workspace with its own `package.json`, `electrobun.config.ts`, and `src/bun/index.ts`. Electrobun 2 builds through Hutch: the first `electrobun` command downloads the toolchain and projects the SDK into `apps/desktop/.hutch/devkit` (gitignored), which the desktop `tsconfig.json` extends. The desktop shell reuses `apps/web` during development and bundles its built static output for release builds. Use a separate backend or no backend for desktop apps; `--backend self` emits server routes inside `apps/web` and cannot be bundled as static desktop assets.
...
```json
{
  ...
  "addons": ["<pwa|tauri|electrobun|starlight|fumadocs|biome|lefthook|husky|mcp|turborepo|nx|vite-plus|ultracite|oxlint|opentui|wxt|skills|evlog|axiom|none>"] ,
...
Each application owns an `.env.schema` and a generated `src/env.ts`. Varlock generates the local `ENV` accessor during installation. Run the root `env:generate` script after changing a schema; do not edit generated types. This uses `@generateTsTypes(path=./src/env.ts, exposeEnv=local)`, as recommended for separate application schemas in [Varlock's monorepo guide](https://varlock.dev/guides/monorepos/).

Shared database and authentication packages accept configuration and database clients from their owning app. Database command-line tools import the app's database settings through `packages/db/.env.schema`.

...
- If you choose `--backend self` with Next.js, TanStack Start, Nuxt, SvelteKit, SolidStart, or Astro, API routes live inside `apps/web` (no `apps/server`).
...
```json
{
  ...
  "addons": ["<pwa|tauri|electrobun|starlight|fumadocs|biome|lefthook|husky|mcp|turborepo|nx|vite-plus|ultracite|oxlint|opentui|wxt|skills|evlog|sentry|none>"] ,

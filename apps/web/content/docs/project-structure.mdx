...
- API=connectrpc: `packages/api` with `proto/`, `src/gen`, and Connect middleware in server; web uses `@connectrpc/connect-query`
...
- `packages/api`: when `--api` is not `none` (non-Convex). For `--api connectrpc` this includes `proto/`, `src/gen` (generated from proto), and services; the web app uses Connect Web + TanStack Query.

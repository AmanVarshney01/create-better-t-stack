...
export async function runTRPCTest(config: TestConfig): Promise<TestResult> {
	...
	try {
		...
		// Change to .smoke directory before creating project
		...
		const options: CreateInput = {
			...
			...coreStackDefaults,
			...config, // Spread all config options (this will override defaults)
			// Handle empty arrays specially - convert empty arrays to ["none"] for CLI compatibility
			...(config.addons !== undefined && {
				addons:
					config.addons.length === 0 ? (["none"] as Addons[]) : config.addons,
			}),
		...
		const result = await caller.init([projectName, cleanOptions]);

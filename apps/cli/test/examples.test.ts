	...
	describe("All Example Types", () => {
		for (const example of EXAMPLES) {
			...
			it(`should work with ${example} example in appropriate setup`, async () => {
				...
				// Choose compatible frontend for each example
				if (example === "ai") {
					config.frontend = ["tanstack-router"]; // AI not compatible with solid
				} else {
					config.frontend = ["tanstack-router"]; // Todo works with any
				}

 ...
 try {
  const argv = await yargs(hideBin(process.argv))
   ...
   .option("backend", {
    ...
    choices: ["hono", "express", "next", "elysia", "fastify"],
   })
   .option("runtime", {
    type: "string",
    describe: "Runtime",
...
function processAndValidateFlags(
 ...
 if ((includesNuxt || includesSvelte) && effectiveApi === "trpc") {
  consola.fatal(
   `tRPC API is not supported with '${includesNuxt ? "nuxt" : "svelte"
   }' frontend. Please use --api orpc or remove '${includesNuxt ? "nuxt" : "svelte"
   }' from --frontend.`,
  );
  process.exit(1);
 }

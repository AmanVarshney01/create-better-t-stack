...
Better Auth 1.7.3 removes the account `issuer` requirement introduced in 1.7.0–1.7.2. Generated auth schemas use the provider/account identity pair; requiring `issuer` breaks email signup. The Varlock runtime smoke test verifies schema push, signup, and an authenticated session against the generated SQLite/Drizzle app. See the [official 1.7 upgrade guide](https://better-auth.com/docs/guides/1-7-upgrade-guide).

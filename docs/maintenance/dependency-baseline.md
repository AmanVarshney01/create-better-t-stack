...
PR preview builds run under `pull_request` with read-only permissions and no persisted checkout credentials. Publishing runs in a separate job inside `release.yaml`, triggered by `workflow_run`, so npm uses the existing trusted publisher workflow identity without an npm token: it checks the current PR/commit/label and archive identities, then publishes only the four expected packages with lifecycle scripts disabled. No PR code executes in the publishing job. The archive validator was checked with valid packages and nine invalid artifact scenarios; a live publication requires the workflow to be merged to the default branch.
